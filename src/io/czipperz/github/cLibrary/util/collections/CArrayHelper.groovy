...
final class CArrayHelper {
	 ...
	 */
	static <T extends Object> T[] condense(T[] t, T delete) {
	 ...
	 */
	static <T extends Object> T[] condense(T[] t) {
		Objects.requireNonNull(t);
		List<T> result = toArrayList(t);
		for(int i = 0; i < t.length; i++)
			if(!(t[i] == null))
	...
	static <T extends Object> List<T> addAll(List<T> a, List<T> t) {
		for(T i : t)
			a.add(i);
		return a;
	}

	static <T extends Object> List<T> addAll(List<T> a, T[] t) {
		for(T i : t)
			a.add(i);
		return a;
	}

	static <T extends Object> ArrayList<T> toArrayList(T[] t) {
	...
	static <T extends Object> T[] toArray(List<T> t) {
		return (T[]) t.toArray();
	}

	static <E extends Object> E[] snip(List<E> a, int x, int width) {
	...
	static <E extends Object> E[] snip(E[] a, int x, int width) {
		E[] result = (E[]) new Object[width];
	...
	static Object[] snipTo(List<? extends Object> a, int x, int endX) {
	...
	static boolean[] snip(boolean[] a, int x, int width) {
	...
	static char[] snip(char[] a, int x, int width) {
	...
	static double[] snip(double[] a, int x, int width) {
	...
	static float[] snip(float[] a, int x, int width) {
	...
	static int[] snip(int[] a, int x, int width) {
	...
	static long[] snip(long[] a, int x, int width) {
	...
	static <T extends Object> ArrayList<T> createArrayList(T object) {
		ArrayList<T> a = new ArrayList<T>();
		a.add(object);
		return a;
	}

	static <T extends Object> T[] createArray(T object) {
		T[] t = (T[]) new Object[1];
		t[0] = object;
		return t;
	}
	 ...
	 */
	static <T extends Object> boolean isOverlap(List<T>[] items) {
	 ...
	 * @return Returns true if an object is measured as equivalent between the lists.
	 */
	static <T extends Object> boolean isOverlap(T[][] items) {
		for (int i = 0; i < items.length - 1; i++)
